...
#       model_name="gemini-2.5-flash", # Using the faster model
...
#     return {"message": "FastAPI Itinerary Generator is running"}
...
#         num_days = data.get("num_days", 3) # Default to 3 days if not provided
#         budget = data.get("budget") # Optional, can be None or empty string
...
#         try:
#             # Ensure num_days is a reasonable integer
#             num_days = int(num_days)
#             if not 1 <= num_days <= 7: # Limit to 1-7 days for practicality
...
#         # Aim for ~4 activities per day, min 3, max 5
#         min_activities = num_days * 3
#         max_activities = num_days * 5
#         target_activities = num_days * 4 # Ask for a target number within the range

#         # --- Construct Budget Instruction ---
#         budget_instruction = ""
#         if budget:
#             # Try to interpret budget - simple example
...
#             else: # Treat as general preference if not clearly numeric or keyword
...
#              # Be a bit flexible with the exact count from the LLM
...
#              # Stricter check if needed:
#              # if activity_count != target_activities:
#              #     raise ValueError(f"Expected exactly {target_activities} activities, received {activity_count}")
...
#         # Return the full list of generated activities
#         return activities
...
# # --- How to Run ---
# # uvicorn itinerary:app --reload --port 8000
...
router = APIRouter() # <-- Change app = FastAPI() to router = APIRouter()

# --- REMOVED THE CORS MIDDLEWARE ---
# (The main.py app will handle this)

# --- REMOVED THE @app.get("/") ROOT ENDPOINT ---
# (The main.py app has its own root)


@router.post("/api/generate-itinerary") # <-- Change @app.post to @router.post
async def generate_itinerary(request: Request):
    try:
        data = await request.json()
        mood = data.get("mood")
