...
export function TripOverview() {
  ...
  const getActivitiesForDate = (date) => {
    return bookedActivities.filter(activity => activity.date === date)
      .sort((a, b) => a.time.localeCompare(b.time));
  };

  const cancelActivity = (activityId) => {
    setBookedActivities(prev => prev.filter(activity => activity.id !== activityId));
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-2">
        ...
        <p className="text-muted-foreground">Manage your complete travel experience</p>
      </div>
      ...
      <Card className="bg-gray-100">
        <CardContent className="p-6">
          <div className="mb-4">
            <h3 className="font-medium text-gray-900">Journey Details</h3>
            <p className="text-sm text-gray-600">Travel information and booking details</p>
          </div>
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="flex-1">
                <div className="flex items-center space-x-2">
                  <span className="font-medium">{journeyDetails.departure}</span>
                  <span className="text-gray-400">→</span>
                  <span className="font-medium">{journeyDetails.arrival}</span>
                </div>
                <div className="text-sm text-gray-600">
                  {new Date(journeyDetails.departureDate).toLocaleDateString()} - {new Date(journeyDetails.returnDate).toLocaleDateString()}
                </div>
              </div>
            </div>
            <div className="flex items-center space-x-2 text-sm text-gray-600">
              <span>Booking ID:</span>
              <Badge variant="secondary">{journeyDetails.bookingId}</Badge>
            </div>
          </div>
        </CardContent>
      </Card>
            ...
            {groupInfo.map((member, index) => (
              <div key={index} className="flex items-center space-x-4 p-3 bg-white rounded-lg">
                ...
                <div className="flex-1">
                  ...
                  <div className="text-sm text-gray-600">{member.age} years, {member.gender}</div>
                </div>
              </div>
            ))}
          </div>
        ...
        <CardContent className="p-6">
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-sm text-gray-600">Your daily schedule and bookings</p>
            </div>
                          ...
                          <div className="flex-1">
                            <div className="flex items-center space-x-3 mb-2">
                              ...
                              <Badge 
                                ...
                                className={activity.status === 'confirmed' ? 'bg-green-100 text-green-700' : 'bg-orange-100 text-orange-700'}
                              >
                                {activity.status}
                              </Badge>
          ...
          <div className="grid grid-cols-3 gap-4 mt-6 pt-6 border-t">
            <div className="text-center p-3 bg-white rounded-lg">
              ...
              <div className="text-lg font-medium">{bookedActivities.length}</div>
              <div className="text-xs text-gray-600">Total Booked</div>
            </div>
            <div className="text-center p-3 bg-white rounded-lg">
              <IndianRupee className="w-6 h-6 mx-auto mb-2 text-green-600" />
              <div className="text-lg font-medium">
                ₹{bookedActivities.reduce((total, activity) => total + (activity.price || 0), 0).toLocaleString()}
              </div>
                  ...
                  <CardContent className="p-4">
                    <div className="space-y-3">
                      ...
                      <Button 
                        ...
                        size="sm"
                        onClick={() => {
                          const selectedDate = document.getElementById(`date-${index}`).value;
                          const selectedTime = document.getElementById(`time-${index}`).value;
                          const timeFormatted = new Date(`2000-01-01T${selectedTime}`).toLocaleTimeString('en-US', {
                            hour: 'numeric',
                            minute: '2-digit',
                            hour12: true
                          });
      ...
      <div className="flex justify-end">
        <Button size="lg" className="bg-gray-300 text-gray-800 hover:bg-gray-400 px-8">
          Book Now
        </Button>
      </div>
    </div>
